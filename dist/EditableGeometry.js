...
export default class EditableGeometry extends Component {
  ...
  render() {
    try {
      ...
      this.lead.pm.enable();
      this.props.map.fitBounds(this.lead.getBounds(), {
        maxZoom: 17
      });
    } catch (error) {
      console.error(error);
  ...
  componentDidMount() {
    ...
    let markers = [];
    let lines = [];
    let middleMarkers = []; // extract markers(white balls) from all the layers and make an array of markers
